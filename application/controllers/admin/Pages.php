...
public function addpage(){
		...
		//multiple files
		$multiarr=[];
		if(!empty($_FILES['multiimages']['name']))
		{
		 
               // $config['upload_path'] = './uploads/'.$folder.'/';
               // $config['allowed_types'] = 'jpg|jpeg|png|gif';
			      $tempname = $_FILES['multiimages']['tmp_name'];
				 // $filename = $_FILES['multiimages']['name'];
			foreach ($_FILES['multiimages']['name'] as $key => $image) 
			{
			  
			  move_uploaded_file($tempname[$key],'./uploads/'.$folder.'/'.$image);
              if($key==0){
						  $picture = $image;
		...
		//multiple files
            ...
            if(!empty($_FILES['detailimage']['name'])){
                $config['upload_path'] = './uploads/'.$folder.'/';
                $config['allowed_types'] = 'jpg|jpeg|png|gif';
                $config['file_name'] = $_FILES['detailimage']['name'];
                
				//echo $config['upload_path'];die;
                $this->load->library('upload',$config);
                $this->upload->initialize($config);
               // echo $config['upload_path'];die;
                if($this->upload->do_upload('detailimage')){
                    $uploadData = $this->upload->data();
                    $detailimage = $uploadData['file_name'];
                    $userData['detailimage']=$detailimage;
                }else{
                    $upload=0;
                   $uploaderr=$this->upload->display_errors();
            ...
            if(!empty($_FILES['htmlfile']['name'])){
                $config['upload_path'] = './uploads/'.$folder.'/';
                $config['allowed_types'] = 'html|jpg|jpeg|png|gif';
                $config['file_name'] = $_FILES['htmlfile']['name'];
               ...
               // echo $config['upload_path'];die;
                if($this->upload->do_upload('htmlfile')){
                    $uploadData = $this->upload->data();
                    $htmlfile = $uploadData['file_name'];
                    $userData['htmlfile']=$htmlfile;
                }else{
                    $upload=0;
                   $uploaderr=$this->upload->display_errors();
           ...
           if($this->input->post('delimg')){
				if(count($this->input->post('delimg'))>0){ 
				$delImgs=$this->input->post('delimg');
				}
				}
                unset($userData['delimg']);
                if($editid==0){
                    $insert = $this->Page_model->insert_page($userData,$table);
					if(count($multiarr)>0){
						$multidat['project_id']=$insert;
						foreach($multiarr as $imgs){
						$multidat['image']=$imgs;
						  $this->Page_model->insert_page($multidat,'project_images');
						}
					}
				...
				if($delImgs !=""){
				if(count($delImgs)>0){ 
						foreach($delImgs as $delimgss){
						 
						$multiwhere['id']=$delimgss;
						  $this->Page_model->did_delete_row($multiwhere,'project_images');
						} 
					}
				}
...
public function add_agencies($id=0)
        {
                ...
                //----------------
                $listdatas=array('name','image','date','isactive','description','details','seo_url');
                $textdatas=array('name','seo_url');
                $imgdatas=array('image');
                $datedatas=array('date');
                $statdatas=array('isactive');
                $descdatas=array('description');
                $fckdescdatas=array('details');
                $lnktitle='clients';
                $lnkid='id';
                $dropdatas['isactive']=array('0'=>'Active','1'=>'Not Active');
                $editid=$id;
...
public function delete_page($id,$table,$data,$where,$view)
	 ...
	 //add projects
   public function add_projects($id=0)
         {
                 $table='projects';
                 $data='*';
                 $folder='projects';
                 ...
                 //----------------
                 $listdatas=array('name','contractor','location','date','enddate','isactive','client','smalldescription','description');
				// $client=array('client');
                 $textdatas=array('name','contractor','location','client');
				 // $textdatas=array('name','client');
				 //$multiimagesdatas=array('multiimages');
                 //$imgdatas=array('image');
                 $datedatas=array('date','enddate');
                 $statdatas=array('isactive');
                 $descdatas=array('smalldescription');
                 $fckdescdatas=array('description');
                 $lnktitle='projects';
                 ...
                 //$catarr=array();
				 $wherecatstat=array('isactive !='=>2,'category'=>1);
				 $catarr=$this->Page_model->getpage('*','pages',$wherecatstat);
                 $this->add_page($editid,$table,$folder,$where,$data,$title,$catarr,$tabs,$excludearr,$tabsstat,$type,$page,$editvals,$id,$upfolder,$listdatas,$imgdatas,$datedatas,$statdatas,$dropdatas,$textdatas,$lnktitle,$lnkid,$descdatas,$fckdescdatas);
         } 
